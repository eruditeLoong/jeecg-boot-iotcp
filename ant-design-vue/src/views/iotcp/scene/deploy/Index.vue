        ...
        methods: {
            ...
            getSceneConfig (sceneId) {
                ...
                getAction(this.url.getSceneConfig, { id: sceneId }).then(res => {
                    ...
                    if (res.success) {
                        ...
                        console.log(self.sceneConfig.modelFiles)
                        let modelFiles = self.sceneConfig.modelFiles.split(',')
                        for (let i in modelFiles) {
                            let type = modelFiles[i].substring(modelFiles[i].lastIndexOf('.')+1).toLowerCase()
                            let url = window._CONFIG['staticDomainURL'] + "/" + modelFiles[i]
            ...
            loadFbx (url) {
                ...
                loader.load(url, function (object) {
                    ...
                    self.camera.position.set(0-be.max.x*1.5, Math.min(be.max.x,be.max.z), be.max.z*1.5)
