                    ...
                    >
                        <a-tab-pane :key="func.code" :tab="func.name" v-for="(func, index) in model.deviceFunctionList">
                            ...
                            <a-card size="small" style="margin-top: 16px" title="用户操作">
                                <template class="table-page-search-submitButtons" slot="extra" style="float: left;overflow: hidden;">
                                    <a-popconfirm @confirm="execFunction(func.code, !func.execConfig.running)" :title="func.execConfig.running?'确定结束吗?':'确定执行吗?'">
                                        <a-button v-if="!func.execConfig.running" type="primary" ghost>执行</a-button>
                                        <a-button v-if="func.execConfig.running"  type="danger"  ghost>结束</a-button>
                                    </a-popconfirm>
                                </template>
                            ...
                            <a-card size="small" style="margin-top: 16px" title="返回结果">
                                <a-form-item :label="func.outputData.name" :labelCol="labelCol" :wrapperCol="wrapperCol">
                                    <a-textarea :readOnly="true" rows="3" v-decorator="[func.code+'.outputData']"></a-textarea>
                                </a-form-item>
                            </a-card>
                        </a-tab-pane>
                    </a-tabs>
